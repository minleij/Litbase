...
*Version 0.4  ·  Created 2026-07-10  ·  Last updated 2026-07-13  ·  Companion document to Classification_Codebook (task-environment/team dimensions) and Extracted_info.docx*
...
Edge-case note (added v0.4): a human-performance/queuing-simulation model that is empirically calibrated and validated against human-subjects data (fitting distributions to, and checking predictions against, real experimental results), rather than validated primarily through analytical proof or closed-form derivation, may still be coded 3.11 provided equations/derivations remain the article's persistent, generalizable contribution. This is an accepted edge case, not a redefinition of 3.11's core "validated primarily analytically" framing above - note it explicitly as an edge case in the justification whenever it applies, rather than treating it as a clean fit. Motivated by Donmez et al. (2010) and Parasuraman et al. (2005), both of which pair a live user study (coded User Study, 3.1, Primary) with a persistent queuing/workload model of this kind (coded 3.11, Secondary).
...
| --- |
| 2026-07-13 (v0.4): Added an edge-case note to Section 3.11 (Modeling/Theoretical Paper), resolving the proposal deferred in the v0.3 entry above. Donmez et al. (2010) and Parasuraman et al. (2005) are now explicitly accepted as coding 3.11 (Secondary, alongside User Study as Primary) even though their persistent queuing/workload models are validated empirically (by calibration against human-subjects data) rather than primarily analytically, per the user's 2026-07-13 decision. This is scoped as an accepted edge case rather than a change to 3.11's core definition, and coders should flag it as such in the justification whenever it recurs. |