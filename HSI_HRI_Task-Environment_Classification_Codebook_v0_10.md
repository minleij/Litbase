...
*Version 0.10  ·  Created 2026-07-07  ·  Last updated 2026-07-13  ·  Companion document to Extracted_info.docx*
...
| **Distinguish from Known Fixed Task Object (3.5)** | Static/Fixed requires the object's location to be initially unknown to the operator/system and discovered via search; Known Fixed Task Object is used instead when the location is already known/visible to the operator from the outset. |
| **Non-examples** | Multiple scattered targets appearing over time (-> Distributed, 3.1); a single target that relocates after being found (-> Moving, 3.2); a hazard expanding from a point of origin (-> Spreading, 3.3); a change in the system's own internal status such as a battery event (-> Not Applicable here; see Monitoring/Maintenance Task, 4.6); an object whose location is already known to the operator from the start (-> Known Fixed Task Object, 3.5). |

## 3.5 Code: Known Fixed Task Object

| **Definition** | A single task-relevant entity/zone, or multiple simultaneous such entities/zones, whose location(s) is/are already known or visible to the operator/system from the outset of the task - no search or discovery process is required to locate them - and that do not relocate, grow, or otherwise change over the course of the task. Distinguished from Static/Fixed (3.4) specifically by this "already known" property, rather than by instance count. |
| --- | --- |
| **Key indicators** | The object's location is explicitly displayed on a map/interface, stated in the task instructions, or otherwise available to the operator before or at the very start of the task, with no detection/search phase required. Once established, its position remains fixed - it does not move, grow, or spread. May occur as a single instance or as multiple simultaneous known-location instances (e.g. several visible target/obstacle markers present from the start). |
| **Source example** | Bashyal et al. (2008), familiarization experiment: the radiation source's gradient is displayed on the map from the outset, so participants do not need to search for it and only need to navigate the swarm toward the already-visible location. |
| **Generalized examples** | Pre-assigned mission targets whose locations and priorities are already known to the operator at mission launch, distinct from any hazard/threat zones that must instead be tracked as they emerge (Cummings & Mitchell, 2008); multiple objects visible on a table/map from the start of a collective-manipulation or transport task, before any command is given (Patel et al., 2022); target zones marked and visible to the operator throughout a navigation task (Villani et al., 2020); a start and target point within a pre-designed, fully-visible path (Yang et al., 2025). |
| **Distinguish from Static/Fixed Task Object (3.4)** | Known Fixed's defining property is that the object's location is already known to the operator from the outset; Static/Fixed's defining property is that the location is initially unknown and must be discovered via search. Both may involve one or multiple instances - instance count does not distinguish the two codes. |
| **Distinguish from Distributed Task Object (3.1)** | Known Fixed may include multiple simultaneous instances, but all are known/visible from the start; Distributed instances arise intermittently or unpredictably in onset (time and/or location) over the course of the task. |
| **Non-examples** | A source location that must be discovered by scanning or systematic search (-> Static/Fixed, 3.4); a target whose presence/location arises unpredictably or intermittently over the course of the mission (-> Distributed, 3.1); a target that relocates after being found (-> Moving, 3.2). |
...
- If the object appears as multiple separate, scattered instances that are individually static and that arise intermittently or unpredictably over the course of the task -> code Distributed Task Object (3.1).

- If the object (single or multiple simultaneous instances) is already known or visible to the operator/system from the outset of the task, requiring no search to locate, and neither relocates nor grows -> code Known Fixed Task Object (3.5).
...
Eight codes are currently defined in this dimension: Avoidance Task (4.1), Search Task (4.2), Engagement Task (4.3), Pursuit Task (4.4), Defend/Block Task (4.5), Monitoring/Maintenance Task (4.6), Transport/Delivery Task (4.7), and Move Task (4.8). Unlike Section 3, more than one of these codes will often apply to the same article, since a single mission commonly combines several behavioral relations across its phases (e.g. search, then pursue, then engage, while separately avoiding a hazard) - see the Decision Procedure (4.9) for how to assign multiple codes and note which phase/condition each applies to.
...
| **Distinguish from Move Task (4.8)** | Avoidance treats contact with the object as a negative outcome to prevent; Move treats reaching a (different, non-hazardous) known-location object as the neutral goal itself, with no negative consequence for contact. |
...
| **Distinguish from Move Task (4.8)** | Search's defining feature is that the object's location or presence is not yet known and must be discovered; Move presumes the destination's location is already known from the outset, so no search/discovery process is required. |
...
| **Distinguish from Transport/Delivery Task (4.7)** | Engagement's decisive action (e.g. neutralizing, capturing, securing) need not change the object's position; Transport/Delivery's defining requirement is precisely that the object's position changes to a specified destination. |
| **Distinguish from Move Task (4.8)** | Engagement requires a further decisive action (neutralize, capture, secure) once contact with an already-identified object is made; Move's goal is simply reaching a known-location object or point, with arrival alone completing the task. |
| **Non-examples** | Rerouting a robot around a hazard so it is never contacted (-> Avoidance Task, 4.1); positioning robots to keep an adversary away from an asset without acting on the adversary directly (-> Defend/Block Task, 4.5); picking up and relocating an object to a destination (-> Transport/Delivery Task, 4.7). |
...
| **Distinguish from Transport/Delivery Task (4.7)** | Pursuit concerns an object that moves under its own power or external causes, which the operator/system follows without causing its motion; Transport/Delivery concerns an object whose motion is caused by the robot team itself (carrying, pushing, or otherwise relocating it). |
| **Non-examples** | Scanning an area for a not-yet-located target (-> Search Task, 4.2); firing on or capturing a tracked target (-> Engagement Task, 4.3); picking up and carrying an object to a destination (-> Transport/Delivery Task, 4.7). |
...
## 4.7 Code: Transport/Delivery Task

| **Definition** | The operator's or system's required behavior toward the task-relevant object is to physically relocate it from its current position to a specified goal position - the object itself is carried, pushed, or otherwise manipulated and moved by the robot team, rather than being an object the robots travel toward, avoid, or act upon in place. |
| --- | --- |
| **Key indicators** | The task-relevant object is picked up, grasped, pushed, or otherwise physically manipulated and caused to move to a new (often already-known) destination by the robot(s) themselves. Success is measured by whether the object reaches the destination, not by the robot team's own position, a decisive act of neutralization/capture, or continuous tracking of independent motion. The object is not adversarial and is not something to be avoided - relocating it is itself the goal. |
| **Source example** | Patel et al. (2022): operators direct a robot swarm to collectively transport objects - via a finite-state-machine "collective transport" behavior (reach object, approach, push, rotate) - from their current position to an operator-specified goal position. |
| **Generalized examples** | A robot team pushing, carrying, or otherwise collaboratively relocating a box, payload, or other manipulable object from a start location to a goal location as part of a collaborative manipulation task; a robotic arm picking up an object and placing or handing it over at a specified location. |
| **Distinguish from Search Task (4.2)** | Search's goal is detecting/locating an object of unknown position; Transport/Delivery presumes the object's location is already known and instead concerns physically relocating it. |
| **Distinguish from Pursuit Task (4.4)** | Pursuit concerns an object that moves under its own power or external causes, which the operator/system follows without causing its motion; Transport/Delivery concerns an object whose motion is caused by the robot team itself. |
| **Distinguish from Engagement Task (4.3)** | Engagement's decisive action (e.g. neutralizing, capturing, securing) need not change the object's position; Transport/Delivery's defining requirement is precisely that the object's position changes to a specified destination. |
| **Distinguish from Move Task (4.8)** | Transport/Delivery concerns physically relocating a separate object to a destination via the robot team's own manipulation; Move concerns the robot team's own travel to a location, with nothing else being carried or relocated. |
| **Non-examples** | Continuously following an object that moves on its own (-> Pursuit Task, 4.4); capturing, neutralizing, or securing an object in place without relocating it to a specified destination (-> Engagement Task, 4.3); searching for an object of unknown location (-> Search Task, 4.2). |

## 4.8 Code: Move Task

| **Definition** | The operator's or system's required behavior toward the task-relevant object/location is simply to travel to it or reach it, where the object's/location's position is already known from the outset (no search required), contact carries no negative consequence (nothing to prevent), and arrival itself - not any further decisive action - completes the task. |
| --- | --- |
| **Key indicators** | The destination is a Known Fixed Task Object (3.5) or a comparable already-known location, not something discovered via search. The task's success criterion is reaching/arriving at the location, typically as quickly or directly as possible. No negative consequence is attached to contact (ruling out Avoidance, 4.1), and no further decisive action beyond arrival is required (ruling out Engagement, 4.3). |
| **Source example** | Bashyal et al. (2008), familiarization experiment: the radiation source's location is displayed on the map from the outset, and participants must simply guide the swarm to reach that already-visible region as fast as possible; arrival alone completes the task, with no negative consequence for contact and no decisive action taken once there. |
| **Generalized examples** | Directing firefighters/robots toward a known exit or incident-scene point (Naghsh et al., 2008); navigating to a known, visible target zone in a physical-robot experiment (Villani et al., 2020); navigating a microrobotic swarm to a known target point within a vascular phantom (Yang et al., 2025). |
| **Distinguish from Search Task (4.2)** | Move presumes the destination's location is already known to the operator/system; Search's defining feature is that the object's location or presence must first be discovered. |
| **Distinguish from Engagement Task (4.3)** | Move's goal is reaching the location itself; Engagement requires a further decisive action (neutralize, capture, secure) once contact is made. |
| **Distinguish from Avoidance Task (4.1)** | Avoidance treats contact with the object as a negative outcome to prevent; Move treats reaching the location as the (neutral) goal itself. |
| **Distinguish from Transport/Delivery Task (4.7)** | Move concerns the robot team's own travel to a location; Transport/Delivery concerns physically relocating a separate object to a destination. |
| **Non-examples** | Traveling toward a location whose position must first be discovered via search (-> Search Task, 4.2); reaching a target and then taking further decisive action on it (-> Engagement Task, 4.3); routing away from a location to prevent contact (-> Avoidance Task, 4.1); carrying or pushing a separate object to a destination (-> Transport/Delivery Task, 4.7). |

## 4.9 Decision Procedure
...
- If the object's/location's position is already known from the outset (no search required), contact carries no negative consequence, and the goal is simply to travel to/reach it, with arrival itself (not any further action) completing the task -> code Move Task (4.8).
...
- If the required behavior is to physically relocate the object itself (carrying, pushing, or otherwise manipulating it) from its current position to a specified goal position, where the object's own motion is caused by the robot team -> code Transport/Delivery Task (4.7).

- More than one code may apply to a single article if it involves multiple distinct behavioral relations (e.g. searching for a target, then engaging it, while separately avoiding a hazard, or moving to a known point and then engaging a target found there) - assign every code the text supports and note which task/phase each applies to.
...
Scope note on operator-directed computation (added v0.10): Sections 8.1-8.6 all describe mechanisms that compute a robot's own motion, behavior, or task assignment. Testing against the corpus surfaced a distinct, recurring pattern instead - algorithms that compute or optimize something about the human operator, such as scheduling which target the operator should attend to next, or inferring and cueing the operator's own situation awareness - which does not fit any Section 8 code even loosely, since none of 8.1-8.6 take the operator as their object of computation. After review, this project has decided not to add a dedicated code for operator-directed computation at this time (it remains a candidate for a future dimension, alongside human-involvement measurement more generally, if it recurs enough to justify one - see Section 13). Code such mechanisms "Not Applicable" for this dimension rather than raising a fresh "Unclear / Candidate New Code" flag, since Section 8 is, by design, scoped only to computation governing robot behavior, not operator behavior.
...
- If the control mechanism is not described in enough detail to determine this -> "Not Described" (a control mechanism of some kind always conceptually exists, so "Not Applicable" should not normally apply to this dimension). The one standing exception (added v0.10): a computational mechanism whose object is the human operator rather than a robot (see the operator-directed-computation scope note above) -> code "Not Applicable" instead, since this dimension does not conceptually extend to operator-directed computation by design.
...
| **Confined/Enclosed Structure** | The system operates within an enclosed man-made or biological conduit (e.g. pipes, ducts, blood vessels), navigating through its interior rather than an open Air/Land/Maritime/Space environment. |
...
| --- |
| **2026-07-13 (v0.9)** | Added two new codes, motivated by a batch-coding pass of 21 articles during which "Unclear / Candidate New Code" was raised repeatedly for a known-location task-object pattern and an object-relocation behavioral relation with no existing match. Added Section 3.5, Code: Known Fixed Task Object (a single instance, or multiple simultaneous instances, of a task-relevant object whose location is already known/visible to the operator from the outset of the task, rather than discovered via search - distinguished from Static/Fixed Task Object, 3.4, specifically by this "already known" property rather than by instance count; source example: Bashyal et al., 2008, familiarization experiment), renumbering the Section 3 Decision Procedure to 3.6 and adding a cross-reference note to Static/Fixed Task Object (3.4). Added Section 4.7, Code: Transport/Delivery Task (the operator/system's required behavior is to physically relocate the task-relevant object itself, via the robot team's own manipulation, from its current position to a specified goal position, rather than to travel toward, avoid, or act upon it in place - distinguished from Pursuit Task, 4.4, where the object moves under its own power, and from Engagement Task, 4.3, whose decisive action need not relocate the object; source example: Patel et al., 2022), renumbering the Section 4 Decision Procedure to 4.8 and adding cross-reference notes to Pursuit Task (4.4) and Engagement Task (4.3). A related recurring gap raised in the same batch-coding pass - a navigate-to-a-known-target behavioral relation, distinct from Search (location already known) and Engagement (no further action beyond arrival) - along with several one-off proposals (a report/escalate behavioral relation, a formation/cohesion-maintenance behavioral relation, and a confined/enclosed-structure operational domain) were deferred by the user for later consideration and are not reflected in this version. Five articles that prompted some of the deferred proposals (and were judged out of scope for this project's swarm-focused literature review) were removed from the corpus in the same session: single-robot/non-swarm HRI studies (Howard & Cruz, 2006; Paul et al., 2023; Suzuki et al., 2024; Strazdas et al., 2022) and an offline mission-scheduling DSS study with no live robot control (Cushing et al., 2006). |
| --- |
| **2026-07-13 (v0.10)** | Added Section 4.8, Code: Move Task (the operator/system simply travels to or reaches an already-known-location object/point, with no negative consequence for contact and no further decisive action required - arrival alone completes the task), motivated by a gap deferred in v0.9 and raised across four corpus articles: Bashyal et al. (2008, source example), Naghsh et al. (2008), Villani et al. (2020), and Yang et al. (2025). Renumbered the Section 4 Decision Procedure to 4.9, added a Move Task branch to it, and added "Distinguish from Move Task (4.8)" cross-reference rows to Avoidance Task (4.1), Search Task (4.2), Engagement Task (4.3), and Transport/Delivery Task (4.7). Added a new code, Confined/Enclosed Structure, to Section 10 (Operational Domain) - navigation within an enclosed man-made or biological conduit (pipes, ducts, blood vessels), distinct from the open-space Air/Land/Maritime/Space domains - motivated by a gap also deferred in v0.9 and raised by Turco et al. (2024, source example) and Yang et al. (2025). Added a scope note to Section 8 (Computational Characteristics), before 8.1, and a corresponding exception to the 8.7 Decision Procedure's Not-Applicable guidance: operator-directed computation (algorithms that schedule/optimize the human operator's own attention or workload, or infer/cue operator situation awareness, rather than computing robot motion or task allocation) is out of scope for this dimension; per user review, no dedicated code will be added for this pattern, and such mechanisms should be coded "Not Applicable" rather than re-flagged as Unclear. This resolves a recurring flag raised by Cummings & Mitchell (2008), Fortmann (2013), and Frische & Lüdtke (2013). Three one-off proposals from the v0.9 batch-coding pass remain open and are not addressed in this version: a report/escalate behavioral relation (Leijonhufvud et al., 2026), an intruder-classification/deconfliction behavioral relation (Fortmann, 2013), and a formation/cohesion-maintenance behavioral relation (Yang et al., 2025, for the swarm-stability portion of its flag not resolved by the new Move Task code). |