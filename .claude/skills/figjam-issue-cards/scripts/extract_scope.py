"""
Step 1 of the figjam-issue-cards pipeline.
...
summary.issues text (used as a checklist by the quote-extraction subagents,
not quoted directly).
...
Design/Interface Concept Paper) rarely report concrete interface issues from
real use, so including them mostly just produces empty-issues entries. Pass
--study-types with a comma-separated list to widen scope if the user asks
for it (e.g. "User Study,Design/Interface Concept Paper").
"""
import argparse
import json
...
def main():
    ...
    for a in out:
        ...
        print(f"  {a['article_id']}: {n_dims}/{len(dim_keys)} dimensions coded{flag}")
