"""
Step 3 of the figjam-issue-cards pipeline.

Generates one standalone SVG per issue card (plus one legend SVG) from
article_meta.json + issues_data.json. These get rasterized to PNG by
rasterize_cards.py in the next step.

WHY ONE SVG PER CARD, NOT A GRID/SHEET OF SEVERAL CARDS PER FILE:
An earlier version of this pipeline grouped several cards per article into
one bigger "sheet" SVG (a grid layout), meant to be imported into Figma/
FigJam as an editable vector group. Two things went wrong: (1) Figma's SVG
importer badly mangled hand-rolled multi-<text>-element SVGs -- it collapsed
and overlapped sibling <text> lines that render perfectly correctly in any
real browser, apparently not respecting each element's own y position; and
(2) even fixed, a flattened multi-card image can't be pulled apart on a
whiteboard -- the whole point of a card wall is that each issue is
independently movable. So: rasterize to PNG (sidesteps Figma's importer
entirely -- a real browser renders this SVG correctly, see
rasterize_cards.py) AND keep card granularity at one-file-per-card (so each
PNG stays independently draggable once it's on the board). Don't regress
either of these without a specific reason.
...
Usage:
    python generate_cards.py --article-meta <path> --issues-data <path> \
    ...
    python generate_cards.py --dimension-order <path> --out-dir <dir> --legend-only
        ...
        needing article_meta.json/issues_data.json yet)

Writes:
    <out-dir>/<article_id>__<NN>_<slug>.svg   -- one per issue card
    <out-dir>/_legend.svg
...
def render_card_svg(card: dict, dim_meta: dict, dim_orders: dict) -> tuple[str, int, int]:
    ...
    rasterized 1:1 to its own PNG file, one image per issue card."""
    parts = []
    cy = PAD
...
def main():
    ...
    ap.add_argument("--legend-only", action="store_true",
                     help="Only (re)generate _legend.svg -- useful to preview the color scheme "
                          "without needing article_meta.json/issues_data.json yet.")
    args = ap.parse_args()
    ...
    if not args.article_meta or not args.issues_data:
        raise SystemExit("--article-meta and --issues-data are required unless --legend-only is set")

    article_meta = json.loads(Path(args.article_meta).read_text(encoding="utf-8"))
    issues_by_id = json.loads(Path(args.issues_data).read_text(encoding="utf-8"))
    ...
    manifest = []
    for aid, issue_entry in issues_by_id.items():
        meta = meta_by_id.get(aid)
        if not meta:
            continue
        issues = issue_entry.get("issues", [])
        if not issues:
            continue
        ...
        for idx, issue in enumerate(issues, start=1):
            ...
            card_id = f"{aid}__{idx:02d}_{slugify(issue['title'])}"
            (out_dir / f"{card_id}.svg").write_text(svg_str, encoding="utf-8")
            manifest.append({"card_id": card_id, "article_id": aid, "width": w, "height": h, "title": issue["title"]})
